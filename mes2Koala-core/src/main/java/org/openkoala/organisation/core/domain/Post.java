...
public class Post extends Party {
	...
	public static List<Post> findByJob(Job job, Date date) {
		return getRepository().createNamedQuery("findByJob")
				.addParameter("job", job).addParameter("date", date).list();
	}
	...
	public static List<Post> findByOrganization(Organization organization,
			Date date) {
		return getRepository().createNamedQuery("findByOrganization")
	...
	public Set<Employee> getEmployees(Date date) {
		return new HashSet<Employee>(EmployeePostHolding.findEmployeesOfPost(
				this, date));
	}
	 ...
	 * @return
	 */
	public static boolean hasPrincipalPostOfOrganization(
	...
	public void save() {
		...
		/*
		 * if (nameExist()) { throw new NameExistException(); }
		 */

		if (organizationPrincipal
				&& hasPrincipalPostOfOrganization(organization, new Date())) {
			if (getId() != null) {
	...
	private boolean isOrganizationPrincipalBefore() {
		return getRepository().get(Post.class, getId())
				.isOrganizationPrincipal();
	}

	private boolean nameExist() {
		Date now = new Date();
		CriteriaQuery query = getRepository().createCriteriaQuery(Post.class)
				.eq("name", getName()).le("createDate", now)
	...
	private boolean postExist() {
		Date now = new Date();
		CriteriaQuery query = getRepository().createCriteriaQuery(Post.class)
				.eq("organization", organization).eq("job", job)
				.eq("name", getName()).le("createDate", now)
				.gt("terminateDate", now);
