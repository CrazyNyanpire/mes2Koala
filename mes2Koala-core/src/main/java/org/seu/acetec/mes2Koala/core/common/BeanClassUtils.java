...
public class BeanClassUtils {
    ...
    public Map<String, Class<?>> getPropTypes() {
        ...
        for (Map.Entry<String, PropertyDescriptor> each : getPropertyDescriptors()
                .entrySet()) {
    ...
    public Set<String> getReadablePropNamesExclude(
        ...
        List<Class<? extends Annotation>> annotationsExclude = Arrays
                .asList(excludeAnnotations);
        Set<String> results = new HashSet<String>();
    ...
    private boolean methodContainsAnnotation(Method readMethod,
                                             List<Class<? extends Annotation>> annotationsExclude) {
