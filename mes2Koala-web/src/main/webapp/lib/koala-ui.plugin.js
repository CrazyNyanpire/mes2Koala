 ...
 */+ function() {"use strict";
	var Grid = function(element, options) {
		...
		this.itemsMap = {};
		this._initLayout(options.gridheight);
		this._initButtons();
		this._initHead();
		this._initOptions();
		this._initEvents();
	...
	Grid.prototype = {
		Constructor : Grid,
		_initLayout : function(gridheight) {
			!!gridheight==0?gridheight=620:gridheight=gridheight;
			...
			this.gridTableBody.height(this.gridTableBody.height()*$(window).height()/gridheight);
		...
		initSelectRowEvent : function() {
			...
			indexCheckboxs.off('click').on('click', function(e) {
				...
				flagArray.push("check");
			});
			this.gridTableBodyTable.find('tr').off('click').on('click', function() {
				flagArray.push("tr");
				if(!(flagArray[0]=="check"&&flagArray[1]=="tr"&&flagArray.length<3)){
					var $this = $(this);
			...
			this.gridTableBodyTable.find('tr').off('dblclick').on('dblclick', function() {
				var $this = $(this);
			/*	if ($this.hasClass('success')) {
					$this.removeClass('success').find('[data-role="indexCheckbox"]').removeClass('checked');
				} else {
		...
		renderRows : function() {
			...
			for (var i = 0, j = items.length; i < j; i++) {
				...
				if (this.options.isShowIndexCol) {
					trHtml.push('<td');
					if(item.state && item.state === '1'){//测试任务
						trHtml.push(' class="success"');
					}else if(item.state && item.state === '4'){
						trHtml.push(' class="danger"');
					}
					if(item.state && item.state === 1){//ft下单
						trHtml.push(' style="background-color:lightgrey;"');
					}
					trHtml.push(' width="50px;"><div class="checkerbox" indexValue="' + i + '" data-role="indexCheckbox" data-value="' + item[this.options.identity] + '"></div></td>');
				} else {
