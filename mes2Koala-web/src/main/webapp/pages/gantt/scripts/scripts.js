 ...
 */
angular.module('angularGanttDemoApp')
    .controller('MainCtrl', ['$scope', '$http', '$timeout', '$log','$uibModal', 'ganttUtils', 'GanttObjectModel', 'Sample', 'ganttMouseOffset', 'ganttDebounce', 'moment',function($scope, $http, $timeout, $log, $uibModal, utils, ObjectModel, Sample, mouseOffset, debounce, moment) {
        var objectModel;
        var dataToRemove;
                    ...
                    api.directives.on.new($scope, function(directiveName, directiveScope, element) {
                        if (directiveName === 'ganttTask') {
							element.bind('dblclick', function(event) {
                                $scope.open('lg', directiveScope.task.model, { testSyses: $scope.testSyses } );
//                                $scope.open('lg', directiveScope.task.model );
							});
...
/*        // Visual two way binding.
		...
		$scope.reset = function(){
			...
			$http.get('../../TestSys/getTestSys.koala').success(function(result){
				$scope.testSyses = result;
			})
			.error(function(){
				alert("加载测试机台失败！");
			});
			...
			$http.get('../../TestSys/getSchedule.koala').success(function(result){
                var dataTemp = result['data'];
                dataTemp[0]['height'] = '3em';
                dataTemp[0]['sortable'] = false;
                angular.forEach( dataTemp, function( row, dataTemp ){
                	angular.forEach( row['tasks'], function( task, tasks ) {
                		if ( task.state == 0 ) //未测试
                		{
                			//设置甘特条颜色
                			task.color = '#9FC5F8';
                		}
                		else if ( task.state == 1 ) //已测试
                		{
//                			task.color = '#90EE90'; //默认为灰色
                		}
                		else
                		{
                			task.color = '#F1C232';
...
//                dataTemp[0]['classes'] = 'gantt-row-milestone';
//                dataTemp[0]['color'] = '#45607D';
//                $scope.data = $scope.data.concat( result['data'] );
    	...
    	$scope.update = function() {
			$http.post('../../TestSys/updateSchedule.koala', $scope.data ).success(function(result){
                console.info("Saved.");
        ...
        $scope.open = function (size, task, testSyses) {
...
angular.module('angularGanttDemoApp').controller('ModalInstanceCtrl',  [ '$http', '$scope', '$uibModalInstance', 'task', 'testSyses', function ($http, $scope, $uibModalInstance, task, testSyses) {
	
	//alert相关
	$scope.alerts = [
//	    { type: 'success', msg: '保存成功' },
//	    { type: 'danger', msg: '保存失败' }
	];
...
//	    $uibModalInstance.dismiss('cancel');
	...
	//分批中需要用到数据
	$scope.separateFormData = {};
	$scope.separateFormData['id'] = task['id'];
    $scope.separate = function () {
    	var req = {
    			 method: 'POST',
    			 url: '../../ProductionSchedule/separate.koala',
    			 data: $scope.separateFormData,
    	...
    	$http(req).
    	  success(function(data, status, headers, config) {
    		  if ( data['success'] )
    			  ...
    			  $scope.addAlert( { type: 'danger', msg: data['errorMessage'] })
    	  }).
    	  error(function(data, status, headers, config) {
    		  $scope.addAlert( { type: 'danger', msg: '请求失败' } );
    	  });
    };
    ...
    $scope.pause = function () {
    	var req = {
    			 method: 'POST',
    			 url: '../../ProductionSchedule/updateState/' + task['id'] + '.koala',
    			 data: { state: '6' },
    			 headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
    		};
    	$http(req).
