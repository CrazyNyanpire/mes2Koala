...
public class EmployeeController extends BaseController {
	...
	public List<EmployeeDTO> getEmployee(EmployeeDTO example) {
		List<EmployeeDTO> result = new ArrayList<EmployeeDTO>();
		for(int i=0;;i++){
			Page<EmployeeDTO> page = employeeFacade.pagingQueryEmployees(example,i,10);
			result.addAll(page.getData());
			if(i>=page.getResultCount()/10){
				break;
			}
		}
		return result;
	 ...
	 */
    @ResponseBody
    @RequestMapping("/getEmployee-by-org")
    public List<EmployeeDTO> getEmployeeByOrg(EmployeeDTO example,Long organizationId) {
        List<EmployeeDTO> result = new ArrayList<EmployeeDTO>();
        for(int i=0;;i++){
            Page<EmployeeDTO> page = employeeFacade.pagingQueryEmployeesByOrganization(example,organizationId,i,10);
            result.addAll(page.getData());
            if(i>=page.getResultCount()/10){
                break;
            }
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/getEmployeesIdAndName", method = RequestMethod.GET)
	public List<EmployeeVo> getEmployeesIdAndName() {
		return employeeFacade.getEmployeesIdAndName();
	}
