/**
 * Tree
 */+ function($) {"use strict";//把tree-folder改为tree-folder-plus以避免和easyui.css冲突
	...
	Tree.prototype = {
		...
		init : function() {
			...
			$.each(self.$element.find('.tree-folder-plus'), function() {
				var $folder = $(this);
				$folder.on({
					...
					'disSelectCheckBox' : function(e) {
						...
						var treeFolderCheckBox = treeFolder.find('>input');
						if (treeFolderCheckBox.is(':checked')) {
							if (treeFolderContent.find('>.tree-selected').length == 0 && treeFolderContent.find('>.tree-folder-plus>input:checked').length == 0) {
				...
				$folder.find('>input').on('click', function() {
					var data = $folder.find('.tree-folder-plus-header').data();
					if (this.checked) {
						...
						$folder.find('.tree-folder-plus').trigger('selectCheckBox');
						self.selectedDatas[data.id] = data;
					} else {
						$folder.find('.tree-selected').each(function() {
							$(this).click();
						});
						$folder.find('.tree-folder-plus').trigger('disSelectCheckBox');
						delete self.selectedDatas[data.id];
					}
				});
			});
			...
			//父节点点击的时候
			this.$element.on('click', '.tree-folder-plus>input', $.proxy(function(ev) {
				this.checkParent(ev.currentTarget);
			}, this));
			//父节点点击的时候
			this.$element.on('click', '.tree-folder-plus-header', $.proxy(function(ev) {
				this.selectParent(ev.currentTarget);
			}, this));
			//打开子节点
			this.$element.on('click', '.tree-folder-plus-header i', $.proxy(function(ev) {
			...
			this.$element.on('mousedown', '.tree-folder-plus-name,.tree-item-name', $.proxy(function(ev) {
				this.popMenu(ev.currentTarget, ev);
			}, this));
			//添加鼠标右键事件监听
			this.$element.on('contextmenu', '.tree-folder-plus-header,.tree-item', $.proxy(function(ev) {
		...
		render : function() {
			...
			this.$element.find('.tree-folder-plus').show();
		...
		populate : function(data, $el) {
			...
			$.each(data, function(index, value) {
				...
				if (!self.isEmpty(value.children) || value.type == 'parent') {
					...
					$entity.find('.tree-folder-plus-name').html(value.menu.title);
					$entity.find('.tree-folder-plus-header').data(value.menu);
					$entity.attr("id", value.menu.id);
					self.populate(value.children, $entity.find(".tree-folder-plus-content:eq(0)"));
				    if (value.menu.open) {
						$entity.find('.glyphicon-folder-close').removeClass('glyphicon-folder-close').addClass('glyphicon-folder-open');
						$entity.find(".tree-folder-plus-content:eq(0)").show();
					} else {
						$entity.find('.glyphicon-folder-open').removeClass('glyphicon-folder-open').addClass('glyphicon-folder-close');
						$entity.find(".tree-folder-plus-content:eq(0)").hide();
					}
					//这里加上检测是否有自定义图标
					if (value.menu.icon) {
						$entity.children('.tree-folder-plus-header').children("i").addClass(value.menu.icon).attr("icon", value.menu.icon);
					}
					if (value.menu.checked) {
						$entity.find('>input').prop('checked', true);
						var data = $entity.find('.tree-folder-plus-header').data();
						self.selectedDatas[data.id] = data;
					}
				} else {
		...
		createParent : function(value) {
			...
			//如果采用复选框
			if (this.options.useChkBox) {
				$node = $('<div class="tree-folder-plus checkbox" style="display:none;">' + '<input type="checkbox" value="' + value + '">' + '<div class="tree-folder-plus-header">' + '<i class="glyphicon glyphicon-folder-close"></i>' + '<div class="tree-folder-plus-name"></div></div><div class="tree-folder-plus-content"></div>' + '<div class="tree-loader" style="display:none"></div></div>');
			} else {
				$node = $('<div class="tree-folder-plus" style="display:none;">' + '<div class="tree-folder-plus-header"><i class="glyphicon glyphicon-folder-close"></i>' + '<div class="tree-folder-plus-name"></div></div><div class="tree-folder-plus-content"></div>' + '<div class="tree-loader" style="display:none"></div></div>');
			}
		...
		createChildren : function(value) {
			...
			//如果采用复选框
			if (this.options.useChkBox) {
				$node = $('<div class="tree-item checkbox" style="display:none;">' + '<input type="checkbox" value="' + value + '">' + '<div class="tree-item-name"></div></div>');
			} else {
				$node = $('<div class="tree-item" style="display:none;">' + '<i class="glyphicon glyphicon-list-alt"></i><div class="tree-item-name"></div></div>');
			}
			return $node;
		},
		...
		checkParent : function(el) {
			...
			if ($input.is(':checked')) {
				...
				if (folder.find('>.tree-selected').length == 0 && folder.find('>.tree-folder-plus>input:checked').length == 0) {
		...
		toggleFolder : function(e) {
			...
			var $folder = $el.closest('.tree-folder-plus');
			if ($el.hasClass('glyphicon-folder-close')) {
				$el.removeClass('glyphicon-folder-close').addClass('glyphicon-folder-open');
				$folder.find('.tree-folder-plus-content:eq(0)').show();
				this.$element.trigger('opened', {
					element : $el.parent(),
					data : $el.parent().data()
				});
			} else {
				$el.removeClass('glyphicon-folder-open').addClass('glyphicon-folder-close');
				$folder.find('.tree-folder-plus-content:eq(0)').hide();
		...
		popMenu : function(el, ev) {
			...
			if (ev.which == 3) {
				...
				var $p = null;
				//临时节点，存储parent
				if ($el.hasClass("tree-folder-plus-name")) {
					$p = $el.parent().parent();
		...
		addChildren : function(data) {
			...
			if (data.type && data.type == 'parent') {
				...
				$entity.find('.tree-folder-plus-name').html(menu.title);
				$entity.find('.tree-folder-plus-header').data(menu);
				$entity.on('click', {
					element : $(this)
				}, function(e) {
			...
			node.parent().find('>.tree-folder-plus-content').append($entity).show();
	...
	$.fn.tree = function(option, value) {
		...
		var $set = this.each(function() {
			...
			if (!data)
				$this.data('koala.tree', ( data = new Tree(this, options)));
		...
		//添加判断是否可拖放
		if (option.draggable) {
			//			var $node = $(this).find(".tree-folder-plus,.tree-item");
			var $node = $(".tree");
			$node.sortable({
				items : ".tree-folder-plus,.tree-item"
			});
