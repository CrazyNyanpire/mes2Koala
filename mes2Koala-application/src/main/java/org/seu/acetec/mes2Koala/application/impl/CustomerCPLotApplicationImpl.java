...
public class CustomerCPLotApplicationImpl extends
	...
	public CPRuncard createCPRuncard(CPRuncardTemplate cpRuncardTemplate) {
		...
		cpRuncard.setSignedTime(cpRuncardTemplate.getSignedTime());
		...
		cpSpecialFormTemplateTarget
				.setMCP_Cover1Sheet(cpSpecialFormTemplateSource
						.getMCP_Cover1Sheet());
		cpSpecialFormTemplateTarget
				.setMCP_Cover1SheetStatus(cpSpecialFormTemplateSource
						.getMCP_Cover1SheetStatus());
		cpSpecialFormTemplateTarget.setSheet1(cpSpecialFormTemplateSource
				.getSheet1());
		cpSpecialFormTemplateTarget.setSheet1Status(cpSpecialFormTemplateSource
				.getSheet1Status());
		cpSpecialFormTemplateTarget.setSheet2(cpSpecialFormTemplateSource
				.getSheet2());
		cpSpecialFormTemplateTarget.setSheet2Status(cpSpecialFormTemplateSource
				.getSheet2Status());
		cpSpecialFormTemplateTarget.setCP1Sheet(cpSpecialFormTemplateSource
				.getCP1Sheet());
		cpSpecialFormTemplateTarget
				.setCP1SheetStatus(cpSpecialFormTemplateSource
						.getCP1SheetStatus());
		cpSpecialFormTemplateTarget.setCP2Sheet(cpSpecialFormTemplateSource
				.getCP2Sheet());
		cpSpecialFormTemplateTarget
				.setCP2SheetStatus(cpSpecialFormTemplateSource
						.getCP2SheetStatus());

		cpSpecialFormTemplateTarget
				.setMap_Shift1Sheet(cpSpecialFormTemplateSource
						.getMap_Shift1Sheet());
		cpSpecialFormTemplateTarget
				.setMap_Shift1SheetStatus(cpSpecialFormTemplateSource
						.getMap_Shift1SheetStatus());
	...
	public void order(Long customerCPLotId, CPLot cpLot, Long cpInfoId,
		...
		if (StringUtils.isEmpty(cpLot.getShipmentProductNumber())) {
			cpLot.setShipmentProductNumber(customerCPLot
		...
		cpLotApplication.createCheckedCPLot(cpLot, false, true); // 创建CPLot，检查前端回传的编号
		...
		customerCPLot.setCpInfo(cpInfo);
		BeanUtils.copyProperties(sbb, customerCPLot);
		customerCPLot.setOrderUser(sbb.getLastModifyEmployNo());
		customerCPLot.setOrderDate(sbb.getLastModifyTimestamp());
	...
	private Long innerOrder(CustomerCPLot customerCPLot, SaveBaseBean sbb) {
		sbb.setCreateEmployNo(sbb.getLastModifyEmployNo());
		sbb.setCreateTimestamp(sbb.getLastModifyTimestamp());
...
/*			cpWafer.setInternalWaferCode(cpCustomerWafer.getWaferCode() + "-"
	...
	public List<Long> batchOrder(Long[] customerCPLotIds,
			Map<String, Integer> messages, Long cpInfoId, SaveBaseBean sbb) {
		...
		if (cpInfo.getProcessTemplate() == null) {
			throw new RuntimeException("选择的PID没有绑定流程！");
		}
		if (Strings.isNullOrEmpty(cpInfo.getProcessTemplate().getContent())) {
		...
		try {
			for (CustomerCPLot customerFTLot : customerCPLots) {
				Long cpLotId = innerOrder(customerFTLot, sbb);
	...
	public void deleteOrder(CPLot cpLot) {
		...
		customerLot.setOrderUser(null);
		customerLot.setOrderDate(null);
		customerLot.setLastModifyEmployNo(cpLot.getLastModifyEmployNo());
		customerLot.setLastModifyTimestamp(cpLot.getLastModifyTimestamp());
