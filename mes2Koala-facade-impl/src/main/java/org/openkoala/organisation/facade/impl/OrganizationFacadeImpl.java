...
import org.seu.acetec.mes2Koala.facade.dto.vo.OrganizationVo;
...
public class OrganizationFacadeImpl implements OrganizationFacade {
    ...
    @Override
    public Object getOrganizationEmpolyeeTree() {
        StringBuilder jpql = new StringBuilder("SELECT NEW org.seu.acetec.mes2Koala.facade.dto.vo.OrganizationVo"
                + "(r.id, olm.commissioner.id, r.name) "
                + "FROM OrganizationLineManagement olm LEFT JOIN olm.responsible r "
                + "WHERE olm.commissioner is null AND olm.toDate > :queryDate AND olm.fromDate <= :queryDate ORDER BY r.id ASC");

        OrganizationVo top = (OrganizationVo) getQueryChannelService().createJpqlQuery(jpql.toString()).addParameter("queryDate", new Date()).singleResult();

        jpql = new StringBuilder("SELECT NEW org.seu.acetec.mes2Koala.facade.dto.vo.OrganizationVo"
                + "(r.id, olm.commissioner.id, r.name) "
                + "FROM OrganizationLineManagement olm LEFT JOIN olm.responsible r "
                + "WHERE olm.commissioner is not null AND  olm.toDate > :queryDate AND olm.fromDate <= :queryDate ORDER BY r.id ASC");

        List<OrganizationVo> all = getQueryChannelService().createJpqlQuery(jpql.toString()).addParameter("queryDate", new Date()).list();
        LinkedHashMap<Long, OrganizationVo> map = new LinkedHashMap<Long, OrganizationVo>();
        for (OrganizationVo organizationDTO : all) {
            map.put(organizationDTO.getId(), organizationDTO);
        }
        map.put(top.getId(), top);
        for (OrganizationVo organizationDTO : map.values()) {
            Long pid = organizationDTO.getPid();
            if (pid == null || map.get(pid) == null) {
                continue;
            }
        ...
        setEmployeesForOrganizations(top);
        return top;
    }

    private void setEmployeesForOrganizations(OrganizationVo vo) {
        if (vo == null)
            return;
        vo.setEmployeeVos(employeeFacade.findEmployeesByOrganizationId(vo.getId()));
        if (vo.getChildren() == null)
            return;
        for (OrganizationVo child : vo.getChildren()) {
            setEmployeesForOrganizations(child);
        }
    }
}
