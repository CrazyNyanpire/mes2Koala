...
public class ProcessTemplateAssembler {
	...
	public static ProcessTemplateDTO toDTO(ProcessTemplate processTemplate) {
		...
		result.setTestingTemplateDTOs(TestingTemplateAssembler.toDTOs(processTemplate.getTestingTemplates()));
		result.setAcetecAuthorizationDTOs(
				AcetecAuthorizationAssembler.toDTOs(processTemplate.getAcetecAuthorizations()));
		result.setSpecialFormDTO(SpecialFormAssembler.toDTO(processTemplate.getSpecialForm()));
	...
	public static List<ProcessTemplateDTO> toDTOs(Collection<ProcessTemplate> processTemplates) {
	...
	public static ProcessTemplate toEntity(ProcessTemplateDTO processTemplateDTO) {
		...
		result.setAcetecAuthorizations(
				AcetecAuthorizationAssembler.toEntities(processTemplateDTO.getAcetecAuthorizationDTOs()));
...
/*		// for test Howard
		...
		RuncardNote runcardNoteIQC = new RuncardNote();
		List<String> noteOfIQC = new ArrayList<String>();
		noteOfIQC.add("注意事項(Notes):");
		noteOfIQC.add("1.产品烘烤后48H内进入铝箔袋真空包装");
		noteOfIQC.add("3.烘烤完成之后，核对铝箔袋标签、实物与流程单信息，保持三者一致。                                   出烤箱人员(Operator):");
		runcardNoteIQC.setNodeName("IQC");
		runcardNoteIQC.setNodeNote(noteOfIQC);
		note.put(runcardNoteIQC.getNodeName(), runcardNoteIQC);//RuncardNote中的nodeName虽然可以主动更新，但是为了防止覆盖需要赋值key

		RuncardNote runcardNoteBaking = new RuncardNote();
...
*/
		 ...
		 */
		if (processTemplateDTO.getTestIds() != null && processTemplateDTO.getTestIds().size()>0 
			...
			for(int i=0;i<processTemplateDTO.getTestIds().size();i++){
				...
				tList.add(tDTO);
				result.setTestingTemplates(TestingTemplateAssembler.toEntities(tList));
			}
		}
		// LHB
		List<AcetecAuthorization> acetecAuthorizations = new ArrayList<AcetecAuthorization>();
		List<Long> authorizationIds = processTemplateDTO.getAcetecAuthorizationIds();
		...
		//
		result.setSpecialForm(SpecialFormAssembler.toEntity(processTemplateDTO.getSpecialFormDTO()));

/*		//设置note Howard
		//若processTemplateDTO中没有noteDTO则新建一实例
		Map<String, RuncardNoteDTO> noteDTO = processTemplateDTO.getNoteDTO() == null ? 
				new HashMap<String, RuncardNoteDTO>() : processTemplateDTO.getNoteDTO();
		...
		if ( processTemplateDTO.getTestingTemplateDTO() != null )
			process = process.replace("test", result.getTestingTemplate().getContent());
		...
		//寻找每个模板中Flow节点
		for ( String flow : flowOfFT ){
			//如果该节点包括在process中，则需要存储
			if ( process.contains(flow) ){
				//如果需要保存的节点已经存在，则复制到新建的note当中，否则新建
				if ( noteDTO.containsKey(flow) ){
	...
	public static List<ProcessTemplate> toEntities(Collection<ProcessTemplateDTO> processTemplateDTOs) {
