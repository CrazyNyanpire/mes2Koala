...
public class FTLotFacadeImpl implements FTLotFacade {
	...
	public InvokeResult getInternalLotFT(Long id) {
		return InvokeResult
				.success(FTLotAssembler.toPageVo(application.get(id)));
	}
	...
	@Override
	public Page<FTLotPageVo> pageQueryFTPage(FTLotPageVo queryVo,
			int currentPage, int pageSize) {
		List<Object> conditionVals = new ArrayList<>();
		StringBuilder jpql = new StringBuilder(
				"select _internalLot from FTLot _internalLot   where 1=1 ");
		...
		if (queryVo.getLotNumber() != null
				&& !"".equals(queryVo.getLotNumber())) {
			jpql.append(" and _internalLot.internalLotNumber like ?");
			conditionVals.add(MessageFormat.format("%{0}%",
					queryVo.getLotNumber()));
		}
		...
		if (queryVo.getCurrentState() != null
				&& !"".equals(queryVo.getCurrentState())) {
			switch (queryVo.getCurrentState()) {
			case "进行批次":
				jpql.append(" and _internalLot.currentState <> '已经完成' ");
				break;
			case "完结批次":
				jpql.append(" and _internalLot.currentState = '已经完成' ");
				break;
			case "HOLD":
				jpql.append(" and _internalLot.holdState  = 'Hold' ");
				break;
			default:
				jpql.append(" and _internalLot.currentState like ?");
				conditionVals.add(MessageFormat.format("%{0}%",
						queryVo.getCurrentState()));
			}
		...
		return new Page<FTLotPageVo>(pages.getStart(), pages.getResultCount(),
				pageSize, FTLotAssembler.toPageVos(pages.getData()));
	}

	public Page<FTLotDTO> pageQueryFTLot(FTLotDTO queryVo, int currentPage,
			int pageSize) {
		List<Object> conditionVals = new ArrayList<Object>();
		StringBuilder jpql = new StringBuilder(
				"select _fTLot from FTLot _fTLot   where 1=1 ");
		if (queryVo.getCreateTimestamp() != null) {
		...
		if (queryVo.getCurrentState() != null
			...
			switch (queryVo.getCurrentState()) {
			case "进行批次":
				jpql.append(" and _fTLot.currentState <> '已经完成' ");
				break;
			case "完结批次":
				jpql.append(" and _fTLot.currentState = '已经完成' ");
				break;
			case "HOLD":
				jpql.append(" and _fTLot.holdState  = 'Hold' ");
				break;
			default:
				jpql.append(" and _fTLot.currentState like ?");
				conditionVals.add(MessageFormat.format("%{0}%",
		...
		Page<FTLot> pages = getQueryChannelService()
				.createJpqlQuery(jpql.toString()).setParameters(conditionVals)
				.setPage(currentPage, pageSize).pagedList();
		return new Page<FTLotDTO>(pages.getStart(), pages.getResultCount(),
				pageSize, FTLotAssembler.toDTOs(pages.getData()));
	}

	@Override
	public InvokeResult getPlaceHold(Long id) {
		PlaceHolderCP placeHolder = placeHolderApplication.findByCPLot(id);
		return InvokeResult.success();
	}
	...
	public InvokeResult getProcessTemplateByFtLotId(Long id) {
		try {
			...
			String content = CustomerFTLotAssembler.toDTO(
					ftLot.getCustomerFTLot()).getProcessTemplateContent();
