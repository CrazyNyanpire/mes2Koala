...
public class ProductionScheduleAssembler {
	...
	public static ProductionScheduleDTO toDTO(
		 ...
		 * result.setLastModifyTimestamp
		 * (productionSchedule.getLastModifyTimestamp());
		 * result.setCreateEmployNo (productionSchedule.getCreateEmployNo());
		 * result.setLastModifyEmployNo
		 * (productionSchedule.getLastModifyEmployNo()); result.setLogic
		 * (productionSchedule.getLogic());
		 */

		result.setPlanedStartTimestamp(productionSchedule
				.getPlanedStartTimestamp() == null ? ""
				: getScheduleDate(productionSchedule.getPlanedStartTimestamp()));
		result.setPlanedEndTimestamp(productionSchedule.getPlanedEndTimestamp() == null ? ""
				: getScheduleDate(productionSchedule.getPlanedEndTimestamp()));
		result.setActualStartTimestamp(productionSchedule
				.getActualStartTimestamp() == null ? ""
				: getScheduleDate(productionSchedule.getActualStartTimestamp()));
		result.setActualEndTimestamp(productionSchedule.getActualEndTimestamp() == null ? ""
				: getScheduleDate(productionSchedule.getActualEndTimestamp()));
		...
		if (productionSchedule instanceof FTProductionSchedule) {
			...
			try {
				FTLot ftLot = ((FTProductionSchedule) productionSchedule)
						.getFtLot();
				result.setLotCurrentState(ftLot.getCurrentState());
				result.setLotHoldState(ftLot.getHoldState());
				result.setCustomerNumber(ftLot.getCustomerFTLot()
						.getCustomerNumber());
				result.setInternalProductNumber(ftLot.getFtInfo()
						.getInternalProductNumber());
				result.setPackingLot(ftLot.getCustomerFTLot().getPackingLot());
			...
			try {
				CPLot cpLot = ((CPProductionSchedule) productionSchedule)
						.getCpLot();
				result.setLotCurrentState(cpLot.getCurrentState());
				result.setLotHoldState(cpLot.getHoldState());
				result.setCustomerNumber(cpLot.getCustomerCPLot()
						.getCustomerNumber());
				result.setInternalProductNumber(cpLot.getCustomerCPLot().getCpInfo()
						.getInternalProductNumber());
				result.setPackingLot(cpLot.getCustomerCPLot().getPackingLot());
			} catch (NullPointerException e) {
				result.setLotCurrentState("无法获取");
				result.setLotHoldState("无法获取");
			}
		...
		try {
			...
			result.setTestSysName(productionSchedule.getTestSys()
					.getPlatformNumber());
		} catch (NullPointerException e) {
		...
		result.setCustomerProductNumber(productionSchedule
		...
		result.setPlannedTimeTakes(df.format(productionSchedule
				.getPlannedTimeTakes()));
		result.setActualTimeTakes(productionSchedule.getActualTimeTakes() == null ? "0"
				: df.format(productionSchedule.getActualTimeTakes()));

		return result;
	}

	public static List<ProductionScheduleDTO> toDTOs(
			Collection<ProductionSchedule> productionSchedules) {
	...
	/**
	 * 调用者只应在修改时调用此方法 调用者需要保证testSys的ID不为空
	 * 
	 * @param productionScheduleDTO
	 * @return
	 * @deprecated
	 */
	public static ProductionSchedule toEntity(
			ProductionScheduleDTO productionScheduleDTO) {
		if (productionScheduleDTO == null) {
			return null;
		}
		ProductionSchedule result = new ProductionSchedule();
		if (productionScheduleDTO.getId() != null)
			result = productionScheduleApplication.get(productionScheduleDTO
					.getId());
		result.setVersion(productionScheduleDTO.getVersion());
		/*
		 * result.setCreateTimestamp
		 * (productionScheduleDTO.getCreateTimestamp());
		 * result.setLastModifyTimestamp
		 * (productionScheduleDTO.getLastModifyTimestamp());
		 * result.setCreateEmployNo (productionScheduleDTO.getCreateEmployNo());
		 * result.setLastModifyEmployNo
		 * (productionScheduleDTO.getLastModifyEmployNo()); result.setLogic
		 * (productionScheduleDTO.getLogic());
		 */

		result.setPlanedStartTimestamp(setScheduleDate(productionScheduleDTO
				.getPlanedStartTimestamp()));
		// result.setPlanedEndTimestamp(
		// MyDateUtils.addHours(setScheduleDate(productionScheduleDTO.getPlanedStartTimestamp()),
		// 10));
		/*
		 * result.setActualStartTimestamp (
		 * setScheduleDate(productionScheduleDTO.getActualStartTimestamp()));
		 * result.setActualEndTimestamp (
		 * setScheduleDate(productionScheduleDTO.getActualEndTimestamp()));
		 */
		...
		if (productionScheduleDTO.getTestSysId() != null)
			result.setTestSys(testSysApplication.get(productionScheduleDTO
					.getTestSysId()));

		return result;
	}

	public static List<ProductionSchedule> toEntities(
			Collection<ProductionScheduleDTO> productionScheduleDTOs) {
	...
	private static Date setScheduleDate(String dateString) {
		if (dateString == null)
			...
			return MyDateUtils.str2Date(dateString,
					MyDateUtils.DefFormatProductionScheduleString);
	}
	...
	private static String getScheduleDate(Date date) {
		if (date == null)
			...
			return MyDateUtils.formaterDate(date,
					MyDateUtils.DefFormatProductionScheduleString);
	}
}
