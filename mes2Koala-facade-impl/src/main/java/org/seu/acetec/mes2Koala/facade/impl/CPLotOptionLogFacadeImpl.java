...
public class CPLotOptionLogFacadeImpl implements CPLotOptionLogFacade {
	...
	public InvokeResult getCPLotOptionLog(Long id) {
		return InvokeResult.success(CPLotOptionLogAssembler.toDTO(application
				.getCPLotOptionLog(id)));
	}

	public InvokeResult creatCPLotOptionLog(CPLotOptionLogDTO cPLotOptionLogDTO) {
		application.creatCPLotOptionLog(CPLotOptionLogAssembler
				.toEntity(cPLotOptionLogDTO));
		return InvokeResult.success();
	}

	public InvokeResult updateCPLotOptionLog(CPLotOptionLogDTO cPLotOptionLogDTO) {
		application.updateCPLotOptionLog(CPLotOptionLogAssembler
				.toEntity(cPLotOptionLogDTO));
		return InvokeResult.success();
	}
	...
	public List<CPLotOptionLogDTO> findAllCPLotOptionLog() {
		return CPLotOptionLogAssembler.toDTOs(application
				.findAllCPLotOptionLog());
	}

	public Page<CPLotOptionLogDTO> pageQueryCPLotOptionLog(
			CPLotOptionLogDTO queryVo, int currentPage, int pageSize) {
		List<Object> conditionVals = new ArrayList<Object>();
		if (queryVo.getLotId() != null) {
			CPLot cpLot = cpLotapplication.get(queryVo.getLotId());
			if (cpLot.getParentSeparationId() != null
					|| !StringUtils.isEmpty(cpLot.getParentIntegrationIds())) {
				queryVo.setLotId(null);
				queryVo.setLotNo(cpLot.getSourceParentSeparationNo());
			}
		}
		StringBuilder jpql = new StringBuilder(
				"select _cPLotOptionLog from CPLotOptionLog _cPLotOptionLog   where 1=1 ");
		...
		if (queryVo.getLotNo() != null && !"".equals(queryVo.getLotNo())) {
			jpql.append(" and _cPLotOptionLog.cpLot.internalLotNumber like ?");
			conditionVals.add(MessageFormat.format("%{0}%",
					queryVo.getLotNo()));
		}
		//jpql.append(" order by _cPLotOptionLog.createTimestamp desc");
		Page<CPLotOptionLog> pages = getQueryChannelService()
				.createJpqlQuery(jpql.toString()).setParameters(conditionVals)
				.setPage(currentPage, pageSize).pagedList();

		return new Page<CPLotOptionLogDTO>(pages.getStart(),
				pages.getResultCount(), pageSize,
				CPLotOptionLogAssembler.toDTOs(pages.getData()));
	}
